...
export class FacturaService {
  ...
  consultarPorEmpresa(empresaId: number, pag: number, cant: number): Observable<Respuesta> {
    return this.http.get(environment.host + urn.ruta + urn.factura + urn.consultarPorEmpresa + urn.slash + empresaId + urn.slash + pag + urn.slash + cant, options).pipe(
      map(response => response as Respuesta),
      catchError(err => {
        return throwError(()=>err);
      }));
  ...
  consultarFiltroPorEmpresa(filtro: string, empresaId: number, pag: number, cant: number): Observable<Respuesta> {
    return this.http.get(environment.host + urn.ruta + urn.factura + urn.consultarFiltroPorEmpresa + urn.slash + filtro + urn.slash + empresaId + urn.slash + pag + urn.slash + cant, options).pipe(
      map(response => response as Respuesta),
      catchError(err => {
        return throwError(()=>err);
      }));
